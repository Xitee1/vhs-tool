...
# -- tbc-tools compatibility ----------------------------------------------------

_OLD_METADATA_TOOL = "ld-export-metadata"
_NEW_METADATA_TOOL = "tbc-export-metadata"


def needs_compat_shim(probe_output: str) -> bool:
    """Decide from `<appimage> tbc-export-metadata --version` output whether the
    tbc-tools build uses the new tool names.

    tbc-tools builds from 2026 on renamed ld-export-metadata to
    tbc-export-metadata, and their AppImage launcher falls back to the
    ld-analyse GUI for unknown tool names — which would stall the export on
    the metadata step (tbc-video-export <= 0.1.8 uses the old name). Only the
    new builds answer to the new name, so its presence in the output is the
    positive signal; old builds fall back to ld-analyse or error out.
    """
    return _NEW_METADATA_TOOL in probe_output


def probe_tbc_tools(tbc_tools: Path) -> str:
    """Return the AppImage's version output for the new metadata tool name.

    Returns "" when the probe fails; callers then keep the previous behavior
    of passing the AppImage through unchanged.
    """
    try:
        proc = subprocess.run(
            [str(tbc_tools), _NEW_METADATA_TOOL, "--version"],
            capture_output=True,
            text=True,
            timeout=60,
            check=False,
...
def write_compat_shim(tbc_tools: Path, directory: Path) -> Path:
    """Write a wrapper script that maps the old metadata tool name to the new one."""
    appimage = shlex.quote(str(tbc_tools.resolve()))
    shim = directory / "tbc-tools-compat.sh"
    shim.write_text(
        "#!/bin/sh\n"
        f"# Generated by vhs-tool: tbc-video-export invokes {_OLD_METADATA_TOOL},\n"
        f"# which newer tbc-tools builds renamed to {_NEW_METADATA_TOOL}.\n"
        f"if [ $# -eq 0 ]; then exec {appimage}; fi\n"
        'tool="$1"\n'
        "shift\n"
        f'if [ "$tool" = "{_OLD_METADATA_TOOL}" ]; then tool="{_NEW_METADATA_TOOL}"; fi\n'
        f'exec {appimage} "$tool" "$@"\n'
    )
    shim.chmod(0o755)
    return shim
...
def cmd_export(args: argparse.Namespace) -> int:
    ...
    if should_run("video"):
        print("\nExporting FFV1...")
        needs_shim = needs_compat_shim(probe_tbc_tools(tbc_tools))
        with tempfile.TemporaryDirectory(prefix="vhs-tool-") as tmpdir:
            tbc_tools_bin: Path = tbc_tools
            if needs_shim:
                print(
                    f"  Note: this tbc-tools build renamed {_OLD_METADATA_TOOL} to "
                    f"{_NEW_METADATA_TOOL} — using a compat shim"
                )
                tbc_tools_bin = write_compat_shim(tbc_tools, Path(tmpdir))
            export_cmd = [
                tbc_export,
                "--tbc-tools-appimage",
                tbc_tools_bin,
